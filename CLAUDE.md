...
* `_data/projects.yml` — drives Selected Work. One entry per project:
  ...
  `url` (the canonical home — usually GitHub, but `ttsim-blackhole`'s is a Hugging Face
  Space; the link label is just `url | remove: "https://"`, so any host renders fine), and
  optional `site` (a live demo → renders a "Live site" link).
  **Order is editorial, not chronological** — newest/most substantial first.
  ...
  `poster` for clips. **`media_alt` is interpolated into an HTML attribute and goes through
  `| escape`; never remove that filter.** Cards are generated by `script/make_card.py` from
  a spec in `script/cards/` — see 2026-09-07 below. The `chart` kind has no generator and
  no users.
...
**2026-09-07 — tt-gozer gets its microsite, plus tt-cv-agency and a Hugging Face Space.**
Prompt: *"tt-gozer now has a microsite to help make a good image from and link to as well as
GH. also will find a new repo tt-cv-agency underway. While there's no GH repo yet there will
be soon of this too: https://huggingface.co/spaces/episod/ttsim-blackhole"*

26 entries now. Three changes:

**tt-gozer** gained `site:` and its microsite screenshot **replaced the `gozer status`
terminal card**; `assets/img/projects/tt-gozer.svg` was deleted (recover with
`git show e636361:assets/img/projects/tt-gozer.svg`). The microsite is a green-phosphor
"Containment Log" generated from the lease ledger — off the site's teal palette, but it is
the project's own Ghostbusters identity, the same argument that justifies tt-tnt's `logo`.

**`url` is no longer always a GitHub URL.** `ttsim-blackhole` has no repo yet — it lives as
a Hugging Face Space — so its `url` is the Space and its `site` is the running app at
`episod-ttsim-blackhole.hf.space`. This needed no template change: the label is
`url | remove: "https://"`, so it renders as `huggingface.co/spaces/…`. **When the GitHub
repo lands, move `url` to it and keep the Space as `site`.**

**`script/make_card.py` now exists, and it is the fix for a documented mistake.** The
2026-08-19 note complains that the deleted `chart` SVG had no committed generator and had to
be dug out of git history. tt-cv-agency ships no images at all, so it needed a `card` — and
rather than hand-writing another orphan SVG, the generator is committed with its spec in
`script/cards/tt-cv-agency.json`. Regenerate with:

    python3 script/make_card.py script/cards/tt-cv-agency.json assets/img/projects/tt-cv-agency.svg

It encodes both card gotchas as code and comments: leading fixed at 1.23x the font size (or
box-drawing verticals render dashed), and each coloured run positioned at its own absolute
`x` computed from its column, so columns stay aligned across colour changes instead of
depending on the renderer's kerning. The card's text is **verbatim** from tt-cv-agency's
README — the Phase 1 CC-sweep table. That rule is not stylistic: a card stands in for a
screenshot, so inventing a number in one would be inventing a result.

**Sizing a microsite screenshot is set by capture width, not crop height.** The first
tt-gozer capture was 1264px wide, and at the 632px column that is 50% scale — its 13px body
text renders at 6.5px and is unreadable. Cropping height does nothing for that. Two fixes,
both applied: capture no wider than needed, and **crop the dead margin off the centred
content panel** — `crop=984:822:140:28` on a 1264-wide shot took it to 64% scale with all
five stat tiles intact. Check where the panel edges actually are before choosing the crop;
this site's microsites centre a ~980px panel in whatever window they are given.

**The `ttsim-blackhole` figure claims the kernel runs, so that was verified rather than
assumed:** `POST /execute` against the live Space with the editor's own Hello Tensor kernel
returns `exit_code: 0` and `PASSED`, in ~2.1 simulated seconds. `GET /health` reports which
backends are actually up (`ttsim-bh: true`, the other two false) — worth checking before
writing anything about that Space, since its figure is a pre-run screenshot and the claim
about output lives only in `media_note`.
